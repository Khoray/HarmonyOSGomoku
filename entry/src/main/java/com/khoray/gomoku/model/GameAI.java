...
public class GameAI {
    ...
    public GameAI(int width, int height) {
        mWidth = width;
        mHeight = height;
    ...
    public Game.Move getNextCoordinate(int[][] map) {
        int maxpSum = 0;
        int maxcSum = 0;
        ...
        if (maxcValue > maxpValue) {
            c = new Game.Move(whiteRow, whiteCollum, 0);
        } else {
            c = new Game.Move(blackRow, blackCollum, 0);
        }
        return c;
    }
}
